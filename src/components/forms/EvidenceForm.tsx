      ...
      render: (v, set, next) => (
        <>
          {ASSUMPTION_LANGUAGE.test(v.content) ? (
            <p className="mb-3 rounded-[8px] border-2 border-gold/60 bg-gold-soft px-3 py-2 text-sm">
    ...
    },
    {
      ...
      render: (v, set) => (
        <div className="space-y-3">
          ...
          <label className="mt-2 inline-flex cursor-pointer items-center gap-2 rounded-[10px] border-2 border-dashed border-line-strong px-4 py-3 text-sm font-semibold">
          ...
          {v.photo ? <img src={v.photo.dataUrl} alt="Evidence preview" className="max-h-48 rounded-[8px] border-2 border-ink" /> : null}
